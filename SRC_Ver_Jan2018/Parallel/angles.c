...
#pragma omp parallel
#pragma omp for private(i,j,k,Ang1,Ang2,Ang3, AngleAtom1,AngleAtom2,AngleAtom3)
...
for(i=0;i<NumAngles;i++)
   for(j=0;j<9;j++)
      if( usrptr[i].K2 == PosibAngs[j].K2 ){    //This one parameter is enough to specify an angle
         usrptr[i].type = PosibAngs[j].type;
         //usrptr++;
         break;
         }
