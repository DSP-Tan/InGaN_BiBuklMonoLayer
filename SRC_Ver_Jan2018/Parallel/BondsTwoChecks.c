...
int getParams 	( char datafile[] , double *cell );
...
double cutoff, cell[3], *charge;       //cutoff is the Bonded potentials cutoff, this is used to determine bond and angle topology. 
                                       //It is 2.4 in the case of the model used here. The array cell contains the cell size in x,y and z. The pointer charge points to 
                                       //the memory location of the charges on each atom.
char dest[160],dest1[160],supercell[160], Potentials[160];	//This will be arrays containing the names of different files, two output files and two input files.
...
natoms = getParams (supercell , cell );
printf( "\n\nThe number of atoms is %d\n\n", natoms );
...
/* With the number of atoms obtained by getParams, we can now see what size our arrays to contain the coords need to be
...
printf("\n\nMakebonds\n\n");
bondptr = makebonds(Potentials,natoms,x_,y_,z_,ptr,type,num_types,namespt,cutoff,cell, &NumberOfBonds,bondcoeffs,&NumberOfBondTypes); 
printf("\n\nMake Angles \n\n");
...
{	
	...
	while ( fscanf( fpoint, "%s %lf %lf %lf %d %d %d", name,&x,&y,&z, &f1,&f2,&f3 ) == 7 )
		natoms++;
